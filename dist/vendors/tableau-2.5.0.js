  ...
  (function (global) {
    ...
    ss.mkEnum = function (asm, typeName, values, namedValues) {
      ...
      result.isInstanceOfType = function (instance) {
        return _typeof(instance) === (namedValues ? 'string' : 'number');
      };
    ...
    ss.getInstanceType = function (instance) {
      if (!ss.isValue(instance)) throw new ss_NullReferenceException('Cannot get type of null'); // NOTE: We have to catch exceptions because the constructor
    ...
    ss.initInterface(ss_IReadOnlyCollection, {
      get_count: null,
      contains: null
    }, [ss_IEnumerable]); //#include "TimeSpan.js"
    ///////////////////////////////////////////////////////////////////////////////
    // IEqualityComparer
    ...
    ss.initInterface(ss_IReadOnlyList, {
      get_item: null
    }, [ss_IReadOnlyCollection, ss_IEnumerable]); // #include "IDictionary.js"
  ...
  (function () {
    ...
    var $tab__Utility = global.tab._Utility = ss.mkType($asm, 'tab._Utility', null, null, {
      ...
      isDate: function Utility$IsDate(value) {
        if (_typeof(value) === 'object' && ss.isInstanceOfType(value, ss.JsDate)) {
          return true;
  ...
  (function () {
    ...
    var $tab__CustomViewImpl = ss.mkType($asm, 'tab._CustomViewImpl', function (workbookImpl, name, messagingOptions) {
      ...
      $saveAsync: function CustomViewImpl$SaveAsync() {
        ...
        var deferred = new tab._Deferred(); // prepare the parameter

        var param = {};
        param['api.customViewParam'] = this.$presModel; // send the command
      ...
      _saveNewAsync: function CustomViewImpl$SaveNewAsync(workbookImpl, messagingOptions, name) {
        var deferred = new tab._Deferred(); // prepare the parameter, if cv is null, show the original view

        var param = {};
        param['api.customViewName'] = name; // send the command
        ...
        var returnHandler = $tab__CustomViewImpl.$createCustomViewCommandReturnHandler('api.SaveNewCustomViewCommand', deferred, function (result) {
          $tab__CustomViewImpl._processCustomViewUpdate(workbookImpl, messagingOptions, result, true); // get the first view on the list. There should be only one in the list
      ...
      $processOneCustomViewUpdate: function CustomViewImpl$ProcessOneCustomViewUpdate(workbookImpl, messagingOptions, customViewsInfo, customView, shouldUpdateList) {
        ...
        var customViewImpl = $tab__CustomViewImpl._createNew(workbookImpl, messagingOptions, customView, defaultCustomViewId); // TFSID 783674: Since we use the custom view name as the unique key when storing custom views, an
      ...
      $addObjects: function DashboardImpl$AddObjects(zones, findSheetFunc) {
        ...
        for (var i = 0; i < zones.length; i++) {
          ...
          if (zones[i].objectType === 'worksheet') {
            ...
            var index = this.$worksheets.get__length(); // Contained sheets are always AUTOMATIC size.

            var size = tab.SheetSizeFactory.createAutomatic(); // Contained sheets aren't active - the containing sheet is active.

            var isActive = false; // The contained sheet is hidden if it's not one of the published sheets.
    ...
    var $tab__DataSourceImpl = global.tab._DataSourceImpl = ss.mkType($asm, 'tab._DataSourceImpl', function (name, isPrimary) {
      ...
      processDataSource: function DataSourceImpl$ProcessDataSource(dataSourcePm) {
        var dataSourceImpl = new $tab__DataSourceImpl(dataSourcePm.name, dataSourcePm.isPrimary); // Add the fields.
    ...
    var $tab__SheetImpl = global.tab._SheetImpl = ss.mkType($asm, 'tab._SheetImpl', function (sheetInfoImpl, workbookImpl, messagingOptions) {
      ...
      changeSizeAsync: function SheetImpl$ChangeSizeAsync(newSize) {
        ...
        if (this.$size.behavior === newSize.behavior && newSize.behavior === 'automatic') {
          ...
          return deferred.get_promise(); // nothing to do
        }

        var dict = this.$processSheetSize(newSize); // prepare the parameter
        ...
        param['api.maxHeight'] = dict['api.maxHeight']; // call cross domain
      ...
      $normalizeSheetSize: function SheetImpl$NormalizeSheetSize(size) {
        var behavior = tab.PublicEnums.normalizeEnum(tab.ApiSheetSizeBehavior).call(null, size.behavior, 'size.behavior'); // make sure we convert the value to int if not null or undefined
    ...
    var $tab__StoryImpl = global.tab._StoryImpl = ss.mkType($asm, 'tab._StoryImpl', function (sheetInfoImpl, workbookImpl, messagingOptions, storyPm, findSheetFunc) {
      ...
      update: function StoryImpl$Update(storyPm) {
        ...
        var newActiveStoryPointInfoImpl = null; // Initialize the story points. This will preserve any existing state
        ...
        for (var i = 0; i < storyPm.storyPoints.length; i++) {
          ...
          var storyPointInfoImpl = $tab__StoryPointInfoImpl.$ctor(caption, i, storyPointPm.storyPointId, isActive, storyPointPm.isUpdated, this); // Add the new story point info to the collection (remember that in
        ...
        this.$storyPointsInfo.splice(storyPm.storyPoints.length, deleteCount); // See if a new story point has become active.

        var activeStoryPointChanged = ss.isNullOrUndefined(this.$activeStoryPointImpl) || this.$activeStoryPointImpl.get_storyPointId() !== newActiveStoryPointInfoImpl.storyPointId; // If a new story point has become active, mark the old instance as inactive.
      ...
      activateStoryPointAsync: function StoryImpl$ActivateStoryPointAsync(index) {
        ...
        var commandParameters = {};
        commandParameters['api.storyPointIndex'] = index; // Set up the command processing upon completion.
      ...
      revertStoryPointAsync: function StoryImpl$RevertStoryPointAsync(index) {
        ...
        var deferred = new tab._Deferred(); // Create the cross-domain command parameters.
      ...
      $activatePreviousNextStoryPointAsync: function StoryImpl$ActivatePreviousNextStoryPointAsync(commandName) {
        ...
        var commandParameters = {}; // Set up the command processing upon completion.
      ...
      $updateStoryPointInfo: function StoryImpl$UpdateStoryPointInfo(index, newStoryPointPm) {
        ...
        existingImpl.isUpdated = newStoryPointPm.isUpdated; // Update the active StoryPoint if necessary.
      ...
      $updateActiveState: function StoryImpl$UpdateActiveState(newActiveStoryPointPm) {
        ...
        var newActiveIndex = newActiveStoryPointPm.index; // No need to do anything if we didn't switch story points.
        ...
        var containedSheetImpl = $tab__StoryPointImpl.createContainedSheet(newActiveStoryPointPm.containedSheetInfo, this.get_workbookImpl(), this.get_messagingOptions(), this.$findSheetFunc); // Change the active state for the new and the old.
    ...
    var $tab__StoryPointImpl = global.tab._StoryPointImpl = ss.mkType($asm, 'tab._StoryPointImpl', function (storyPointInfoImpl, containedSheetImpl) {
      ...
      this.$containedSheetImpl = containedSheetImpl; // Set the parent pointers back to this instance.
      ...
      createContainedSheet: function StoryPointImpl$CreateContainedSheet(containedSheetInfo, workbookImpl, messagingOptions, findSheetFunc) {
        ...
        var containedSheetType = tab.ApiEnumConverter.convertSheetType(containedSheetInfo.sheetType); // Index doesn't mean anything for a contained sheet.

        var index = -1; // Contained sheets are always AUTOMATIC size.
        ...
        if (containedSheetInfo.sheetType === 'worksheet') {
          ...
          var dashboardImpl = new $tab__DashboardImpl(sheetInfoImpl, workbookImpl, messagingOptions); // Add all of the dashboard objects.
    ...
    var $tab__VizParameters = global.tab._VizParameters = ss.mkType($asm, 'tab._VizParameters', function (element, url, options) {
      ...
      if (ss.isValue(options.height) || ss.isValue(options.width)) {
        this.fixedSize = true; // B87467: when width/height is integer, convert that into 'px'
      ...
      var urlParts = url.split('?'); // remove parameters from the URL, we don't support params in url
      ...
      $constructUrl: function VizParameters$ConstructUrl() {
        ...
        url.push('&:showVizHome=n'); // We don't support mixed mode where the API script file is debug
        ...
        try {
          while ($t1.moveNext()) {
            var entry = $t1.current(); // Ignore values that are handled in other parts of the code
    ...
    var $tab__WorkbookImpl = global.tab._WorkbookImpl = ss.mkType($asm, 'tab._WorkbookImpl', function (vizImpl, messagingOptions, callback) {
      ...
      _setActiveSheetAsync: function WorkbookImpl$ActivateSheetAsync(sheetNameOrInfoOrIndex) {
        ...
        var sheetName = $tab__WorkbookImpl.$extractSheetName(sheetNameOrInfoOrIndex); // check if sheet is one of the embedded worksheet
      ...
      $activateSheetInternalAsync: function WorkbookImpl$ActivateSheetInternalAsync(sheetName, sheetUrl) {
        var deferred = new tab._Deferred(); // Check to see if the sheet is already active.
        ...
        param['api.oldRepositoryUrl'] = this.$activeSheetImpl.get_url(); // call cross domain
        ...
        var returnHandler = new (ss.makeGenericType(tab.CommandReturnHandler$1, [Object]))('api.SwitchActiveSheetCommand', 0, ss.mkdel(this, function (result) {
          this.$vizImpl.$workbookTabSwitchHandler = ss.mkdel(this, function () {
            this.$vizImpl.$workbookTabSwitchHandler = null; // remove the handler

            deferred.resolve(this.$activeSheetImpl.get_sheet()); // return the active sheet
          });
        }), function (remoteError, message) {
          deferred.reject(tab._TableauException.createServerError(message));
        });
      ...
      $getClientInfo: function WorkbookImpl$GetClientInfo(callback) {
        ...
        var returnHandler = new (ss.makeGenericType(tab.CommandReturnHandler$1, [Object]))('api.GetClientInfoCommand', 0, ss.mkdel(this, function (result) {
          this.$processInfo(result); // tell the parent viz we are done loading
      ...
      $processInfo: function WorkbookImpl$ProcessInfo(clientInfo) {
        ...
        this.$vizImpl.set_instanceId(clientInfo.instanceId); // Create all sheets in collection
      ...
      $initializeActiveSheet: function WorkbookImpl$InitializeActiveSheet(clientInfo) {
        var currentSheetName = clientInfo.currentSheetName; // active sheet should be a published sheet or a switched to hidden sheet.
        ...
        if (ss.isValue(this.$activatingHiddenSheetImpl)) {
          ...
          if (baseSheet.sheetType === 'dashboard') {
            ...
            this.$activeSheetImpl = dashboardImpl; // Add all of the dashboard objects.
            ...
            this.$activeSheetImpl = storyImpl1; // Listen to the story point switch event.
      ...
      $createSheetsInfo: function WorkbookImpl$CreateSheetsInfo(clientInfo) {
        ...
        for (var index = 0; index < baseSheets.length; index++) {
          ...
          var sheetName = baseSheet.name; // Create the SheetInfo
      ...
      $getSingleParameterAsync: function WorkbookImpl$GetSingleParameterAsync(parameterName) {
        var deferred = new tab._Deferred(); // Use the locally cached-parameter if available.
        ...
        var commandParameters = {}; // send the command
      ...
      $changeParameterValueAsync: function WorkbookImpl$ChangeParameterValueAsync(parameterName, value) {
        var deferred = new tab._Deferred(); // check the name and value if this.parameters is available
        ...
        this.$lastChangedParameterImpl = null; // send the command
      ...
      $createDashboardZones: function WorkbookImpl$CreateDashboardZones(zones) {
        ...
        for (var i = 0; i < zones.length; i++) {
          var zone = zones[i];
          var objectType = tab.ApiEnumConverter.convertDashboardObjectType(zone.zoneType); // parse the frame size

          var size = tab.Size.$ctor(zone.width, zone.height); // parse the frame position

          var position = tab.Point.$ctor(zone.x, zone.y); // frame name
    ...
    var $tab__WorksheetImpl = global.tab._WorksheetImpl = ss.mkType($asm, 'tab._WorksheetImpl', function (sheetInfoImpl, workbookImpl, messagingOptions, parentDashboardImpl) {
      ...
      $getDataSourcesAsync: function WorksheetImpl$GetDataSourcesAsync() {
        ...
        var deferred = new tab._Deferred(); // Prepare the command parameters.

        var commandParameters = {};
        commandParameters['api.worksheetName'] = this.get_name(); // send the command
      ...
      $addVisualIdToCommand: function WorksheetImpl$AddVisualIdToCommand(commandParameters) {
        ...
        if (ss.isValue(this.get_parentStoryPointImpl())) {
          var visualId = {};
          visualId.worksheet = this.get_name(); // We always need to have a dashboard parameter, so either use the "real" dashboard
      ...
      $getFilterAsync: function WorksheetImpl$GetFilterAsync(fieldName, fieldCaption, options) {
        ...
        commandParameters['api.filterHierarchicalLevels'] = 0; // no depth details to be consistent

        commandParameters['api.ignoreDomain'] = options.ignoreDomain || false; // send the command
      ...
      $getFiltersAsync: function WorksheetImpl$GetFiltersAsync(options) {
        this.$verifyActiveSheetOrEmbeddedInActiveDashboard();
        options = options || new Object(); // prepare the command parameters
      ...
      $clearFilterInternalAsync: function WorksheetImpl$ClearFilterInternalAsync(fieldName) {
        ...
        this.$addVisualIdToCommand(commandParameters); // Send the command
      ...
      $applyHierarchicalFilterInternalAsync: function WorksheetImpl$ApplyHierarchicalFilterInternalAsync(fieldName, values, updateType, options) {
        ...
        this.$addVisualIdToCommand(commandParameters); // call the client code
      ...
      $getSelectedMarksAsync: function WorksheetImpl$GetSelectedMarksAsync() {
        this.$verifyActiveSheetOrEmbeddedInActiveDashboard(); // prepare the command parameters
      ...
      $selectMarksWithMarksArrayAsync: function WorksheetImpl$SelectMarksWithMarksArrayAsync(marksArray, updateType) {
        ...
        for (var i = 0; i < marksArray.length; i++) {
          var mark = marksArray[i]; // if tupleid exists, use it and don't worry about the rest in the mark object
      ...
      $parseMarksParam: function WorksheetImpl$ParseMarksParam(catNameList, catValueList, hierNameList, hierValueList, rangeNameList, rangeValueList, fieldName, value) {
        var sourceOptions = value; // hierarchical field format: "[Time].[All Time].[1998].[Quarter 1]"
      ...
      $selectMarksWithValuesAsync: function WorksheetImpl$SelectMarksWithValuesAsync(tupleIdList, catNameList, catValueList, hierNameList, hierValueList, rangeNameList, rangeValueList, updateType) {
        ...
        var deferred = new tab._Deferred(); // send the command
    ...
    var $tab_$ParameterImpl = ss.mkType($asm, 'tab.$ParameterImpl', function (pm) {
      ...
      this.$allowableValuesType = tab.ApiEnumConverter.convertParameterAllowableValuesType(pm.allowableValuesType); // TODO we actually have allowables values in the "range" case, do we want them
    ...
    var $tab_JsApiMessageRouter = global.tab.JsApiMessageRouter = ss.mkType($asm, 'tab.JsApiMessageRouter', function () {
      ...
      sendCommand: function sendCommand(T) {
        return function JsApiMessageRouter$SendCommand(source, commandParameters, returnHandler) {
          this.$crossDomainMessager.sendCommand(T).call(this.$crossDomainMessager, source, commandParameters, returnHandler); // jrockwood-2012-12-19:
    ...
    var $tab_VizImpl = global.tab.VizImpl = ss.mkType($asm, 'tab.VizImpl', function (messageRouter, viz, parentElement, url, options) {
      ...
      $calculateFrameSize: function VizImpl$CalculateFrameSize(availableSize) {
        ...
        var width = 0;
        var height = 0; // If it's an exact size, use it. The size of the container is disregarded.
      ...
      $getNewFrameSize: function VizImpl$GetNewFrameSize() {
        var availableSize; // Use the initial available size once and set it to null so we don't use it again
      ...
      $refreshSize: function VizImpl$RefreshSize() {
        ...
        this.$setFrameSize(frameSize.width + 'px', frameSize.height + 'px'); // The act of changing the frame's size might change the available space. For example,
      ...
      $create: function VizImpl$Create() {
        ...
        if (!this.$parameters.fixedSize) {
          this.$initialAvailableSize = tab._Utility.computeContentSize(this.get_$parentElement()); // If the parent element does not have a set width and height, default to an available space
          // of 800 x 600
          ...
          this.$iframe = this.$createIframe(); // Keep the viz invisible until the size of the frame is set
      ...
      $sendVisibleRect: function VizImpl$SendVisibleRect() {
        ...
        var iframeContentRect = tab._Utility.contentRectInDocumentCoordinates(this.get_iframe()); // translate visible rect from this document's coordinates to the iframe's
      ...
      sendScaleFactor: function VizImpl$SendScaleFactor(requestId) {
        var scaleFactor = document.documentElement.clientWidth / window.innerWidth; // These scroll values had meaningful values once upon a time, but were changed to always be 0 when they
      ...
      $invokeAutomaticUpdatesCommandAsync: function VizImpl$InvokeAutomaticUpdatesCommandAsync(command) {
        ...
        var param = {};
        param['api.invokeCommandName'] = command; // call cross domain
      ...
      $handleMarksSelectionChangedEvent: function VizImpl$HandleMarksSelectionChangedEvent(notification) {
        ...
        var activeSheetImpl = this.$workbookImpl.get_activeSheetImpl(); // If the marks occurred on a sheet within the story, then we have to check the
      ...
      $handleParameterChangedEvent: function VizImpl$HandleParameterChangedEvent(notification) {
        ...
        if (!ss.staticEquals(this.$1$ParameterValueChangeField, null)) {
          if (ss.referenceEquals(this.$workbookImpl.get_name(), notification.get_workbookName())) {
            ...
            var parameterName = ss.cast(notification.get_data(), String); // param[2] is the parameter
      ...
      $handleCustomViewsListLoadedEvent: function VizImpl$HandleCustomViewsListLoadedEvent(notification) {
        ...
        var raiseEvents = ss.mkdel(this, function () {
          ...
          this.$raiseCustomViewsListLoad(); // let the programmer know CustomView loaded
      ...
      $handleCustomViewUpdatedEvent: function VizImpl$HandleCustomViewUpdatedEvent(notification) {
        var info = JSON.parse(ss.cast(notification.get_data(), String)); // Create the workbook if does not exist
      ...
      $handleTabSwitchEvent: function VizImpl$HandleTabSwitchEvent(notification) {
        this.$workbookImpl._update(ss.mkdel(this, function () {
          ...
          if (ss.referenceEquals(this.$workbookImpl.get_name(), notification.get_workbookName())) {
            var oldSheetName = notification.get_worksheetName(); // the old sheet name

            var currSheetName = ss.cast(notification.get_data(), String); // the curr sheet name
      ...
      $ensureCalledAfterFirstInteractive: function VizImpl$EnsureCalledAfterFirstInteractive(action) {
        ...
        poll = ss.mkdel(this, function () {
          var now = new Date(); // Call the action if we're ready, time out if we've waited too
      ...
      $createIframe: function VizImpl$CreateIframe() {
        ...
        if (this.$parameters.fixedSize) {
          ...
          ifr.style.width = '1px';
          ifr.style.height = '1px'; // Scrollbars can appear on window resize, and this causes the viz to be sized
      ...
      $getLocalizedTitle: function VizImpl$GetLocalizedTitle() {
        var lang = window.navigator.language; // localized strings copied over from Strings.AccessibilityDataVisualizationTitleAttr
      ...
      $onIframeMouseWheel: function VizImpl$OnIframeMouseWheel(e) {// no-op
      },
      ...
      switchToViz: function VizImpl$SwitchToViz(url) {
        ...
        var param = {};
        param['api.invokeCommandParam'] = url; // send the command
    ...
    var $tableauSoftware_Filter = global.tableauSoftware.Filter = ss.mkType($asm, 'tableauSoftware.Filter', function (worksheetImpl, pm) {
      ...
      getFieldAsync: function Filter$GetFieldAsync() {
        var deferred = new tab._Deferred(); // if field is not instantiated yet,
      ...
      $initializeFromJson: function Filter$InitializeFromJson(pm) {
        ...
        this.$type = tab.ApiEnumConverter.convertFilterType(pm.filterType); // extract the field details info for later use
      ...
      processFiltersList: function Filter$ProcessFiltersList(worksheetImpl, filtersListDict) {
        ...
        for (var $t2 = 0; $t2 < filtersListDict.filters.length; $t2++) {
          ...
          var filterCollectionKey = filterPm1.caption.toString() + '_' + filterPm1.filterType.toString(); //ensure that the user doesn't already have a field that matches the new field i.e. filter_Categorical
